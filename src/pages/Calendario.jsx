...
export default function Calendario() {
  ...
  return (
    <Layout>
      ...
      {!cargando && eventos.length === 0 && <p className="text-ink-soft text-sm">No hay próximos eventos.</p>}
          ...
          <div key={e.id} className="border border-ink/10 rounded-lg bg-white p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="font-mono text-xs text-gold mt-0.5">{formatearFecha(e.fecha_inicio)}</p>
              </div>
              {esEditor && (
                <div className="flex gap-2 font-mono text-xs text-ink-soft shrink-0">
                  <button onClick={() => editar(e)} className="hover:text-petrol">editar</button>
                  <button onClick={() => eliminar(e.id)} className="hover:text-clay">borrar</button>
                </div>
              )}
            </div>
